...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 px-4 text-black bg-orange-200 fixed">
      <div>
        <h1 className="text-3xl font-signature ml-2"> Mark Taborda </h1>
      </div>
      <ul className="hidden md:flex">
        {links.map(({ id, link }) => (
          <li
            key={id}
            className="px-4 cursor-pointer capitalize font-medium text-gray-1000 hover:scale-105 duration-200"
          >
